...
vignette: >
  %\VignetteIndexEntry{GBIF Data Processing}
...
cat("``` bash\n")
cat(readLines("../slurm/gbif_custom_retrieve.sh"), sep = "\n")
cat("\n```")
```
...
We chose the US East region for downlaoding. In this example, records were filtered to include only those from the `Plantae` kingdom and `Tracheophyta` phylum in Sweden. The downloaded dataset is partitioned by `class` and `order` A citation is available within the data folder.
...
if(Download){
  gbif_snapshot_download(bucket_fs, 
                         gbif_snapshot_url, 
                         local_save_dir,
                         filter_level = c("kingdom", "phylum", "countrycode"),
                         filter_value = c("Plantae", "Tracheophyta", "SE"),
...
```{r, echo=FALSE, results='asis'}
cat("``` bash\n")
cat(readLines("../slurm/gbif_snapshot_download.sh"), sep = "\n")
cat("\n```")
```
...
```{r, echo=FALSE, results='asis'}
cat("``` bash\n")
cat(readLines("../slurm/gbif_snapshot_retrieve.sh"), sep = "\n")
cat("\n```")
```
